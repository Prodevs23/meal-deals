...
public function registerNewUser($lastName, $middleName, $firstName, $userName, $email, $phone, $address, $password) {
	//$pasword = md5($pasword);
	$fullName = $lastName . " " . $middleName . " " . $firstName;
	$datum = date("Y/m/d");
	$ret=mysqli_query($this->dbh,"INSERT INTO users (fullname, username, email, phone, address, password, register_date, last_login_date) 
								  VALUES ('$fullName', '$userName', '$email', '$phone', '$address', '$password', '$datum', '$datum')");
	return $ret;
}
